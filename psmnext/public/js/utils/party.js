...
psmnext.utils.get_party_details = function (frm, method, args, callback) {
	if (!method) {
	...
	if (frappe.meta.get_docfield(frm.doc.doctype, "taxes")) {
		if (
			!psmnext.utils.validate_mandatory(
				frm,
...
psmnext.utils.add_item = function (frm) {
	if (frm.is_new()) {
...
psmnext.utils.get_address_display = function (frm, address_field, display_field, is_your_company_address) {
...
psmnext.utils.set_taxes_from_address = function (
	frm,
	...
	frappe.call({
		method: "psmnext.accounts.party.get_address_tax_category",
		args: {
		...
		callback: function (r) {
			if (!r.exc) {
				if (frm.doc.tax_category != r.message) {
					...
					psmnext.utils.set_taxes(frm, triggered_from_field);
				}
			}
		},
	});
...
psmnext.utils.set_taxes = function (frm, triggered_from_field) {
	if (frappe.meta.get_docfield(frm.doc.doctype, "taxes")) {
		if (!psmnext.utils.validate_mandatory(frm, "Company", frm.doc.company, triggered_from_field)) {
			return;
		}
	...
	frappe.call({
		method: "psmnext.accounts.party.set_taxes",
		args: {
			party: party,
			party_type: party_type,
...
psmnext.utils.get_contact_details = function (frm) {
	if (frm.updating_party_details) return;
...
psmnext.utils.validate_mandatory = function (frm, label, value, trigger_on) {
	if (!value) {
		frm.doc[trigger_on] = "";
...
psmnext.utils.get_shipping_address = function (frm, callback) {
	if (frm.doc.company) {
		if (
		...
		frappe.call({
			method: "psmnext.accounts.custom.address.get_shipping_address",
			args: {
				company: frm.doc.company,
