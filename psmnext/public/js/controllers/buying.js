...
psmnext.buying = {
	setup_buying_controller: function() {
		psmnext.buying.BuyingController = class BuyingController extends psmnext.TransactionController {
			setup() {
				super.setup();
				...
				if(this.frm.get_field('shipping_address')) {
					this.frm.set_query("shipping_address", () => {
						if(this.frm.doc.customer) {
							...
							return psmnext.queries.company_address_query(this.frm.doc)
					});
				}
			}
			...
			setup_queries(doc, cdt, cdn) {
				...
				me.frm.set_query('supplier', psmnext.queries.supplier);
				me.frm.set_query('contact_person', psmnext.queries.contact_query);
				me.frm.set_query('supplier_address', psmnext.queries.address_query);

				me.frm.set_query('billing_address', psmnext.queries.company_address_query);
				...
				this.frm.set_query("manufacturer", "items", function(doc, cdt, cdn) {
					...
					return {
						query: "psmnext.controllers.queries.item_manufacturer_query",
			...
			supplier_address() {
				psmnext.utils.get_address_display(this.frm);
				psmnext.utils.set_taxes_from_address(this.frm, "supplier_address", "supplier_address", "supplier_address");
			}
			...
			set_from_product_bundle() {
				...
				this.frm.add_custom_button(__("Product Bundle"), function() {
					psmnext.buying.get_items_from_product_bundle(me.frm);
				}, __("Get Items From"));
			}

			shipping_address(){
				var me = this;
				psmnext.utils.get_address_display(this.frm, "shipping_address",
					"shipping_address_display", true);
			}

			billing_address() {
				psmnext.utils.get_address_display(this.frm, "billing_address",
			...
			manufacturer(doc, cdt, cdn) {
				...
				if(row.manufacturer) {
					frappe.call({
						method: "psmnext.stock.doctype.item_manufacturer.item_manufacturer.get_item_manufacturer_part_no",
						args: {
				...
				frappe.db.get_value("Item", item.item_code, fields)
					.then((r) => {
						if (r.message && (r.message.has_batch_no || r.message.has_serial_no)) {
							...
							new psmnext.SerialBatchPackageSelector(
								me.frm, item, (r) => {
									if (r) {
...
psmnext.buying.link_to_mrs = function(frm) {
	frappe.call({
...
psmnext.buying.get_default_bom = function(frm) {
	$.each(frm.doc["items"] || [], function(i, d) {
		if (d.item_code && d.bom === "") {
			return frappe.call({
				type: "GET",
				method: "psmnext.stock.get_item_details.get_default_bom",
				args: {
...
psmnext.buying.get_items_from_product_bundle = function(frm) {
	var dialog = new frappe.ui.Dialog({
		...
		primary_action(args){
			...
			return frappe.call({
				type: "GET",
				method: "psmnext.stock.doctype.packed_item.packed_item.get_items_from_product_bundle",
				args: {
					row: {
