...
psmnext.landed_cost_taxes_and_charges.setup_triggers("Subcontracting Order");
...
frappe.ui.form.on("Subcontracting Order", {
	...
	set_queries: (frm) => {
		frm.set_query("contact_person", psmnext.queries.contact_query);
		frm.set_query("supplier_address", psmnext.queries.address_query);

		frm.set_query("billing_address", psmnext.queries.company_address_query);

		frm.set_query("shipping_address", () => {
			return psmnext.queries.company_address_query(frm.doc);
		});
	},
	...
	update_subcontracting_order_status(frm, status) {
		frappe.call({
			method: "psmnext.subcontracting.doctype.subcontracting_order.subcontracting_order.update_subcontracting_order_status",
			args: {
				sco: frm.doc.name,
				status: status,
			},
...
psmnext.buying.SubcontractingOrderController = class SubcontractingOrderController {
	setup() {
	...
	make_subcontracting_receipt() {
		frappe.model.open_mapped_doc({
			method: "psmnext.subcontracting.doctype.subcontracting_order.subcontracting_order.make_subcontracting_receipt",
			frm: cur_frm,
...
extend_cscript(cur_frm.cscript, new psmnext.buying.SubcontractingOrderController({ frm: cur_frm }));
