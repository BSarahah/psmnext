...
import psmnext
from psmnext.accounts.deferred_revenue import get_deferred_booking_accounts
...
class JournalEntry(AccountsController):
	...
	if TYPE_CHECKING:
		...
		from psmnext.accounts.doctype.journal_entry_account.journal_entry_account import JournalEntryAccount
	...
	def apply_tax_withholding(self):
		from psmnext.accounts.report.general_ledger.general_ledger import get_account_type_map
	...
	def make_gl_entries(self, cancel=0, adv_adj=0):
		from psmnext.accounts.general_ledger import make_gl_entries
				...
				if not blank_row:
					blank_row = self.append(
						"accounts",
						{
							...
							"cost_center": psmnext.get_default_cost_center(self.company),
						},
					)
...
def get_default_bank_cash_account(company, account_type=None, mode_of_payment=None, account=None):
	from psmnext.accounts.doctype.sales_invoice.sales_invoice import get_bank_cash_account
...
def get_outstanding(args):
	...
	company_currency = psmnext.get_company_currency(args.get("company"))
	due_date = None
...
def get_account_details_and_party_type(account, date, company, debit=None, credit=None, exchange_rate=None):
	...
	company_currency = psmnext.get_company_currency(company)
	account_details = frappe.get_cached_value(
