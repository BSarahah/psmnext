...
frappe.ui.form.on("Item", {
	...
	onload: function (frm) {
		psmnext.item.setup_queries(frm);
	...
	refresh: function (frm) {
		...
		if (frm.doc.has_variants) {
			...
			if (frm.doc.variant_based_on === "Item Attribute") {
				frm.add_custom_button(
					__("Single Variant"),
					function () {
						psmnext.item.show_single_variant_dialog(frm);
					},
					__("Create")
				);
				frm.add_custom_button(
					...
					function () {
						psmnext.item.show_multiple_variants_dialog(frm);
					},
					__("Create")
				);
			} else {
				frm.add_custom_button(
					__("Variant"),
					function () {
						psmnext.item.show_modal_for_manufacturers(frm);
					},
					__("Create")
				);
			}
		...
		psmnext.item.edit_prices_button(frm);
		psmnext.item.toggle_attributes(frm);

		if (!frm.doc.is_fixed_asset) {
			psmnext.item.make_dashboard(frm);
		}
	...
	validate: function (frm) {
		psmnext.item.weight_to_validate(frm);
	},
	...
	is_fixed_asset: function (frm) {
		...
		frappe.call({
			method: "psmnext.stock.doctype.item.item.get_asset_naming_series",
			callback: function (r) {
...
$.extend(psmnext.item, {
	...
	make_dashboard: function (frm) {
		...
		if (frm.doc.is_stock_item) {
			frappe.require("item-dashboard.bundle.js", function () {
				...
				psmnext.item.item_dashboard = new psmnext.stock.ItemDashboard({
					parent: section,
					item_code: frm.doc.name,
					page_length: 20,
					method: "psmnext.stock.dashboard.item_dashboard.get_data",
					template: "item_dashboard_list",
				});
				psmnext.item.item_dashboard.refresh();
			});
		}
	},
	...
	show_modal_for_manufacturers: function (frm) {
		...
		dialog.set_primary_action(__("Create"), function () {
			...
			frappe.call({
				method: "psmnext.controllers.item_variant.get_variant",
				args: data,
				callback: function (r) {
	...
	show_multiple_variants_dialog: function (frm) {
		...
		function make_and_show_dialog(fields) {
			...
			me.multiple_variant_dialog.set_primary_action(__("Create Variants"), () => {
				...
				frappe.call({
					method: "psmnext.controllers.item_variant.enqueue_multiple_variant_creation",
					args: {
						item: frm.doc.name,
			...
			frappe.call({
				...
				callback: function (r) {
					...
					if (r.message) {
						...
						d.hide();
						frappe.call({
							method: "psmnext.controllers.item_variant.create_variant",
							args: {
								item: frm.doc.name,
								args: d.get_values(),
		...
		$.each(d.fields_dict, function (i, field) {
			...
			field.$input
				.on("input", function (e) {
					...
					frappe.call({
						method: "psmnext.stock.doctype.item.item.get_item_attribute",
						args: {
							parent: i,
...
frappe.ui.form.on("UOM Conversion Detail", {
	uom: function (frm, cdt, cdn) {
		...
		if (row.uom) {
			frappe.call({
				method: "psmnext.stock.doctype.item.item.get_uom_conv_factor",
				args: {
					uom: row.uom,
...
frappe.tour["Item"] = [
	...
	},
	{
		...
		description: __(
			"If you are maintaining stock of this Item in your Inventory, psmnext will make a stock ledger entry for each transaction of this item."
		),
	},
	{
